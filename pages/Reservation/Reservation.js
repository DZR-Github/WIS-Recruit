...
Page({
  ...
  data: {
    ...
    multiItems: [
      ...
      ["10:00-11:00", "11:00-12:00", "15:00-16:00", "16:00-17:00", "17:00-18:00", "19:00-20:00", "20:00-21:00"]
    ],
  ...
  CommitReservation() {
    ...
    wx.request({ //发送请求提交面试预约信息
      url: 'http://wisstudio.top/api/user/interview/' + app.globalData.userId,
      data: {
        "interviewTime": TimeDate.slice(0, 11) + TimeDate.slice(14, 19),
      },
      header: {
      ...
      success: (result) => {
        ...
        wx.request({ //发送请求获取当前排队进度
          url: 'http://wisstudio.top/api/user/line/' + app.globalData.userId,
          data: {
  ...
  onShow() {
    wx.request({
      url: 'http://wisstudio.top/api/user/interviewTime',
      method: 'GET',
      success: (result) => {
        // console.log(result.data.data[0].time);
        // console.log(result.data.data[1].time);
        this.setData({
          multiItems: [
            [result.data.data[0].time, result.data.data[1].time],
            ["10:00-11:00", "11:00-12:00", "15:00-16:00", "16:00-17:00", "17:00-18:00", "19:00-20:00", "20:00-21:00"]
          ],
        })
      }
    ...
    wx.request({ //发送请求获取个人信息
      ...
      success: (result) => {
        // console.log(result)
        if (result.data.data.direction == null) {
          this.setData({
            showReservation: true,
            showMessage: "先报名才能预约面试哟！"
          })
        }
